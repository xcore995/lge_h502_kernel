...
#if defined ( TOUCH_MODEL_B2L ) || defined ( TOUCH_MODEL_L80 )
static void WqLpwg_Check(struct work_struct *work_lpwg)
{
	TouchDriverData *pDriverData = container_of(to_delayed_work(work_lpwg),
		TouchDriverData, work_lpwg);
...
{
	TouchDriverData *pDriverData = container_of(to_delayed_work(work_irq), TouchDriverData, work_irq);
	int ret = 0;
	...
	else if( readData.type == DATA_KNOCK_CODE )
	{
		...
		#if defined ( TOUCH_MODEL_B2L ) || defined ( TOUCH_MODEL_L80 )
			cancel_delayed_work_sync(&pDriverData->work_lpwg);
			queue_delayed_work(touch_wq, &pDriverData->work_lpwg, msecs_to_jiffies(200));
		#else
			/* report knock-code event to CFW */
			send_uevent(pDriverData, UEVENT_KNOCK_CODE);
		#endif
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	if( lpwgCmd == LPWG_CMD_CALL ) {
		//TOUCH_DBG("LPWG Call Setting will not processed\n");
		return count;
	}
	#endif
	...
	if( ( lpwgCmd != LPWG_CMD_UPDATE_ALL ) && ( lpwgCmd != LPWG_CMD_CALL ) ) {
		//TOUCH_DBG("LPWG Setting was updated but will be processed later\n");
		mutex_unlock(pMutexTouch);
		return count;
	}
...
static int touch_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	#if !defined ( TOUCH_MODEL_C70 ) /* TBD : temporally block ( C70 is not ready ) */
	if( pDriverData->bootMode == BOOT_MINIOS ) /* MFTS BOOT */
	{
		TOUCH_DBG("MFTS\n");
		if(event == FB_EARLY_EVENT_BLANK)
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if defined ( TOUCH_MODEL_B2L ) || defined ( TOUCH_MODEL_L80 )
	INIT_DELAYED_WORK(&pDriverData->work_lpwg, WqLpwg_Check);
#endif
	...
	/* reset */
	pDeviceSpecificFunc->Reset(NULL);
	...
	TouchEnableIrq();
#if defined ( TOUCH_MODEL_P1S3G )
	TOUCH_LOG("touch_probe pDriverData->icFwInfo.version:%d\n",pDriverData->icFwInfo.version);
	TOUCH_LOG("touch_probe pDriverData->binFwInfo.version:%d\n",pDriverData->binFwInfo.version);
	/* Trigger firmware upgrade if needed */
	if( pDriverData->binFwInfo.isOfficial )
	{
		if( pDriverData->binFwInfo.isOfficial != pDriverData->icFwInfo.isOfficial ||pDriverData->binFwInfo.version != pDriverData->icFwInfo.version)
		{
			pDriverData->useDefaultFirmware = TOUCH_TRUE;
			queue_delayed_work(touch_wq, &pDriverData->work_upgrade, 0);
		}
