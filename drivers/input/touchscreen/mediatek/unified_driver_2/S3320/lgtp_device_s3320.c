...
static int S3320_InterruptHandler(struct i2c_client *client,TouchReadData *pData)
{
	...
	if (regIntStatus == INTERRUPT_MASK_NONE) {
		...
		if (lpwgStatus & 0x1)	/* TCI-1 : Double-Tap */
		{
			...
			if(lpwgFailCount)
			{
				...
				TOUCH_LOG( "TCI1 Debug Index:%d Reason:[%d][%d][%d][%d][%d][%d][%d][%d][%d][%d]\n", lpwgFailIndex,
					FailBuffer[0], FailBuffer[1], FailBuffer[2], FailBuffer[3], FailBuffer[4], FailBuffer[5], FailBuffer[6], FailBuffer[7], FailBuffer[8], FailBuffer[9]);
			}
#endif
		}
		else if (lpwgStatus & 0x2)	/* TCI-2 : Multi-Tap */
		{
			...
			if(lpwgFailCount)
			{
				...
				TOUCH_LOG( "TCI2 Debug Index:%d Reason:[%d][%d][%d][%d][%d][%d][%d][%d][%d][%d]\n", lpwgFailIndex,
					FailBuffer[0], FailBuffer[1], FailBuffer[2], FailBuffer[3], FailBuffer[4], FailBuffer[5], FailBuffer[6], FailBuffer[7], FailBuffer[8], FailBuffer[9]);
			}
#endif
		}/*
#if !defined(ENABLE_SWIPE_MODE)
		else if (lpwgStatus & 0x4)	
		{
			u16 swipe_start_x = 0;
			u16 swipe_start_y = 0;
			u16 swipe_end_x = 0;
