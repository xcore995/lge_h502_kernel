...
void OV8858_Init_Para(void)
{
	...
	ov8858.pvPclk =  (7440); 
	ov8858.videoPclk = (14880);
	ov8858.capPclk = (14880);
...
UINT32 OV8858Open(void)
{
    ...
    sensor_rev = OV8858_read_cmos_sensor(0x302A);
    if((sensor_rev == 0xB0) || (sensor_rev == 0xB1))
        OV8858DB("OV8858 Sensor Rev = R1A\n");
    else if (sensor_rev == 0xB2)
        OV8858DB("OV8858 Sensor Rev = R2A\n");
    else
        OV8858DB("OV8858 Sensor Rev = other(not R1A,R2A)\n");
...
// [LGE_UPDATE_S][yonghwan.lym@lge.com][2015-01-29] LGIT and Sunny module check for Y90
#if defined(TARGET_MT6582_Y90)
extern int module_flag; // 1:Sunny Module, 0:LGIT Module
#endif
// [LGE_UPDATE_E][yonghwan.lym@lge.com][2015-01-29] LGIT and Sunny module check for Y90

//[LGE_UPDATE_S][yonghwan.lym@lge.com][2015-01-02] Multiple EEPROM Suport
...
// [LGE_UPDATE_S][yonghwan.lym@lge.com][2015-01-29] LGIT and Sunny module check for Y90
    ...
    if(module_flag != 0)
    {
        printk("Please check the flag and module. module_flag = %d ...(flag 1 => SUNNY, flag 0 => LGIT)\n",module_flag);
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
#endif
...
//Gionee yanggy 2013-09-03 add for module compbility end
    ...
    OV8858DB("Y90 init Sensor ID = 0x%04x(0x%04x)\n", *sensorID,CurrentSensorIDInList);
    *sensorID =0;
#else
    CurrentSensorIDInList =OV8858_SENSOR_ID;
#endif
	...
	OV8858DB("OV8858GetSensorID enter :\n ");
        ...
        if(CurrentSensorIDInList == OV8858_SENSOR_ID)
        {
            sccb_writeid[0] = OV8858MIPI_WRITE_ID;
        }
        else if(CurrentSensorIDInList == OV8858L_SENSOR_ID)
        {
            sccb_writeid[0] = OV8858MIPI_WRITE_ID1;
        }
        else
        {
            sccb_writeid[0] = OV8858MIPI_WRITE_ID;
            sccb_writeid[1] = OV8858MIPI_WRITE_ID1;
            sccb_writeid[2] = OV8858MIPI_WRITE_ID2;
        }
        backup_slave_addr = ov8858.write_id;

#else
        const kal_uint16 sccb_writeid[] = {OV8858MIPI_WRITE_ID1};
#endif
    ret = FALSE;
    ...
    {
        ov8858.write_id = backup_slave_addr;
        ov8858.read_id = (backup_slave_addr|0x01);
    }

    OV8858DB("Y90 final Sensor ID = 0x%04x\n", *sensorID);
...
//[LGE_UPDATE_E][yonghwan.lym@lge.com][2015-01-02] Multiple EEPROM Suport
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    OV8858SetFlipMirror(IMAGE_HV_MIRROR);
    //TODO~
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE://for factory mode auto testing
            ...
            *pFeatureParaLen=4;
             break;
        //[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-12-24] add FOV value.
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                ...
                OV8858DB("SENSOR_FEATURE_GET_SENSOR_VIEWANGLE\n");
                *pHorFOV = (0x3B)<<16 | (0x3B); // 16:9 = 59, 4:3 = 59 // (59)<<16 | (59) //[LGE_UPDATE][kyunghun.oh@lge.com][2014-02-09] update FOV val.
                *pVerFOV = (0x31)<<16 | (0x31); // 16:9 = 49, 4:3 = 49 // (49)<<16 | (49)
                *pFeatureParaLen = 8;
                break;
            }
        //[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-12-24] add FOV value.
        default:
            break;
    }
    return ERROR_NONE;
