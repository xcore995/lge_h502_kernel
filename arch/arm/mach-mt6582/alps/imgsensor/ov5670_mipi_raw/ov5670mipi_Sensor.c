...
#else
    	framerate = (10 * imgsensor.pclk) / (imgsensor.frame_length * imgsensor.line_length);
    	if(framerate>=280)
    	{
    		framerate=280;
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x5786, 0x01); //0x00);
    ...
    write_cmos_sensor(0x5791, 0x06); //0x04);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                UINT32 *pHorFOV = (UINT32*)feature_return_para_32[0];
                UINT32 *pVerFOV = (UINT32*)feature_return_para_32[1];

                LOG_INF("SENSOR_FEATURE_GET_SENSOR_VIEWANGLE\n");
                *pHorFOV = (0x42)<<16 | (0x42); // 16:9 = 66, 4:3 = 66 // (66)<<16 | (66)
